...
return (
    ...
    <div className="card">
        ...
        {clases.map((clase, index) => (
            <div key={clase.uuid} className="card_subtitle">
            Clases {index + 1} : {clase.className}  {clase.user.name}
            </div>
        ))}
         {signature.map((signature, index) => (
            <div key={signature.uuid} signatureName="card_subtitle">
            Signature {index + 1} : {signature.signatureName}  {signature.user.name}
            </div>
        ))}
        {hours.map((hours, index) => (
            <div key={hours.uuid} hoursName="card_subtitle">
            inicio de hora {index + 1} : {hours.startTime} Hora de salida {hours.endTime}
            </div>
        ))}
        {observations.map((observations, index) => (
            <div key={observations.uuid} observationsName="card_subtitle">
            observation {index + 1} :   {observations.content}
            </div>
        ))}
